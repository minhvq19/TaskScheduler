...
export default function PublicDisplay() {
  ...
  const getSchedulesForStaffAndDay = (staffId: string, day: Date) => {
    ...
    const isBoardMember =
      staffMember &&
      staffMember.department?.name.toLowerCase().includes("giám đốc");
  ...
  const renderWorkScheduleTable = () => {
    ...
    return (
      <div
        className="public-display-table bg-white rounded-lg overflow-hidden shadow-lg"
        style={{ fontFamily: "Roboto, sans-serif" }}
      >
        {/* Table Header */}
        <div
          className="public-display-table-header bg-orange-500"
        ...
        >
          ...
          {days.map((day, index) => {
            ...
            return (
              <div
                key={index}
                className="p-2 text-white font-bold text-center border-r border-orange-600"
              >
        ...
        <div className="public-display-table-body">
          {staff
            ...
            .map((staffMember, rowIndex) => (
              <div
                key={staffMember.id}
                className="public-display-row border-b border-gray-200"
                style={{ display: "grid", gridTemplateColumns: gridTemplate }}
              >
                {/* Staff Name Column */}
                <div
                  className="p-2 bg-teal-700 text-white font-bold border-r border-gray-300 flex items-center"
                  style={{ fontFamily: "Roboto, sans-serif" }}
                >
                  <div>
                    <div
                    ...
                    >
                      {(staffMember as any).positionShort}.{" "}
        ...
        <div
          className="public-display-legend bg-gray-50 p-1 border-t border-gray-300"
          style={{ fontFamily: "Roboto, sans-serif" }}
        >
          <div
            className="text-xs font-bold text-gray-700 mb-1 text-center"
  ...
  const renderMeetingScheduleTable = () => {
    ...
    const weekMeetings = (displayData?.meetingSchedules || []).filter(
      (meeting: any) => {
        ...
        if (meeting.roomId) {
          ...
          console.log(
            "Meeting content:",
            meeting.meetingContent || "NO CONTENT",
          );
          ...
          console.log(
            "UTC start time (formatted):",
            format(utcStartTime, "yyyy-MM-dd HH:mm"),
          );
          console.log(
            "UTC end time (formatted):",
            format(utcEndTime, "yyyy-MM-dd HH:mm"),
          );
          console.log(
            "Meeting start date (UTC):",
            format(meetingStartDate, "yyyy-MM-dd"),
          );
          console.log(
            "Meeting end date (UTC):",
            format(meetingEndDate, "yyyy-MM-dd"),
          );
    ...
    weekMeetings.forEach((meeting: any) => {
      ...
      weekDays.forEach((day) => {
        ...
        if (currentDay >= meetingStart && currentDay <= meetingEnd) {
          if (
            meetingsByRoomAndDate[meeting.roomId] &&
            meetingsByRoomAndDate[meeting.roomId][dateKey]
          ) {
    ...
    console.log(
      "Meetings by room and date:",
      JSON.stringify(meetingsByRoomAndDate, null, 2),
    );

    return (
      <div
        className="public-display-container"
        style={{
          fontFamily: "Roboto, sans-serif",
          height: "100vh",
          width: "100vw",
      ...
      >
        {/* Header */}
        <div
          className="bg-teal-700 text-white py-4 px-6"
          style={{
            height: "120px",
            display: "flex",
        ...
        >
          ...
          <button
            onClick={() =>
              setCurrentScreenIndex((prev) =>
                prev > 0 ? prev - 1 : SCREENS.length - 1,
              )
            }
          ...
          <button
            onClick={() =>
              setCurrentScreenIndex((prev) =>
                prev < SCREENS.length - 1 ? prev + 1 : 0,
              )
            }
            className="bg-orange-600 hover:bg-orange-700 text-white p-2 rounded-full transition-colors"
            style={{
          ...
          <div className="ml-3 text-white" style={{ minWidth: "100px" }}>
            ...
            <div
              style={{ fontSize: "11px", lineHeight: "1.2", color: "#FDE68A" }}
            >
        ...
        <div
          className="public-display-table bg-white"
                    ...
                    <th
                      ...
                      style={{
                        ...
                        borderRight: isLastColumn
                          ? "none"
                          : "1px solid rgb(194 65 12)", // orange-700
                        verticalAlign: "middle",
                      }}
                    >
                      <div>{dayName}</div>
            ...
            <tbody style={{ height: "calc(100% - 60px)" }}>
              {meetingRooms.map((room: any, roomIndex: number) => (
                <tr
                  ...
                  style={{
                    height: `calc((100vh - 180px) / ${meetingRooms.length})`,
                    minHeight: `calc((100vh - 180px) / ${meetingRooms.length})`,
                    maxHeight: `calc((100vh - 180px) / ${meetingRooms.length})`,
                  }}
                >
                  {/* Room Name Column */}
                  <td
                      ...
                      >
                        {dayMeetings.map(
                          (meeting: any, meetingIndex: number) => {
                            ...
                            const now = new Date();
                            const meetingStart = parseLocalDateTime(
                              meeting.startDateTime,
                            );
                            const meetingEnd = parseLocalDateTime(
                              meeting.endDateTime,
                            );
  ...
  const SimpleImageLayout = ({ images }: { images: string[] }) => {
    console.log("SimpleImageLayout render:", {
      ...
      images: images.map((img) => ({
        url: img,
        isAbsolute: !img.startsWith("/"),
      })),
    });

    if (images.length === 1) {
      // Một ảnh - toàn màn hình
      const imgSrc = images[0].startsWith("/")
      ...
      return (
        <div className="w-full h-full flex items-center justify-center p-4">
          <img
            ...
            onLoad={() =>
              console.log("Single image loaded successfully:", imgSrc)
            }
            onError={(e) => {
  ...
  const renderOtherEventsTable = () => {
    ...
    return (
      <div
        className="public-display-table bg-white rounded-lg overflow-hidden shadow-lg h-full"
        style={{ fontFamily: "Roboto, sans-serif" }}
      >
  ...
  return (
    <div
      className="public-display-container bg-teal-900"
      style={{ fontFamily: "Roboto, sans-serif" }}
      data-testid="public-display"
    >
      {/* Header with time in top right */}
      <div
        className="public-display-header bg-teal-900 text-center py-3 relative"
        style={{ fontFamily: "Roboto, sans-serif" }}
      >
        <div
          className="text-yellow-400 font-bold text-[22px]"
          style={{ fontFamily: "Roboto, sans-serif", fontWeight: "700" }}
        >
          NGÂN HÀNG TMCP ĐẦU TƯ VÀ PHÁT TRIỂN VIỆT NAM
        </div>
        <div
          className="text-center font-bold text-[22px] text-[#facc15]"
          style={{ fontFamily: "Roboto, sans-serif", fontWeight: "600" }}
        >
          CHI NHÁNH SỞ GIAO DỊCH 1
        </div>
        <div
          className="font-bold mt-2 text-[#ffffff] text-[44px]"
          style={{ fontFamily: "Roboto, sans-serif", fontWeight: "700" }}
        >
          {(() => {
            const currentScreen = SCREENS[currentScreenIndex];
        ...
        <div className="absolute top-4 right-4 flex items-start gap-3">
          ...
          >
            ...
            <div
              className="text-xs mt-1 text-yellow-300"
              style={{ fontFamily: "Roboto, sans-serif" }}
            >
              Màn hình: {currentScreenIndex + 1}/{SCREENS.length}
            </div>
            <div
              className={`text-xs ${isPaused ? "text-red-300" : "text-yellow-300"}`}
              style={{ fontFamily: "Roboto, sans-serif" }}
            >
