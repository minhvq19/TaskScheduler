...
export default function PublicDisplay() {
  ...
  const renderMeetingScheduleTable = () => {
    ...
    return (
      <div className="public-display-table bg-white rounded-lg overflow-hidden shadow-lg" style={{ fontFamily: 'Roboto, sans-serif', height: 'calc(100vh - 60px)', maxHeight: 'calc(100vh - 60px)' }}>
        <table style={{ width: '100%', height: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', fontFamily: 'Roboto, sans-serif' }}>
          ...
          <tbody style={{ height: 'calc(100% - 60px)' }}>
            {meetingRooms.map((room: any, roomIndex: number) => (
              <tr key={room.id} className="border-b border-gray-200" style={{ height: `calc((100vh - 120px) / ${meetingRooms.length})`, maxHeight: `calc((100vh - 120px) / ${meetingRooms.length})` }}>
                {/* Room Name Column */}
                <td 
